...
export async function POST(request){try{await requirePermission('categories.create');const parsed=schema.parse(await request.json());const {attributeIds,...data}=parsed;const category=await prisma.$transaction(async tx=>{await validateParent(tx,null,data.parentId);const created=await tx.category.create({data});if(attributeIds.length)await tx.categoryAttribute.createMany({data:attributeIds.map(attributeId=>({categoryId:created.id,attributeId}))});return tx.category.findUnique({where:{id:created.id},include})});invalidatePublicCache('categories','products','homepage');return Response.json(category,{status:201})}catch(error){return jsonError(error)}}
export async function PATCH(request){try{await requirePermission('categories.edit');const {id,...input}=await request.json();const parsed=schema.partial().parse(input);const {attributeIds,...data}=parsed;const existing=await prisma.category.findUnique({where:{id},select:{image:true}});const category=await prisma.$transaction(async tx=>{await validateParent(tx,id,data.parentId);await tx.category.update({where:{id},data});if(attributeIds){await tx.categoryAttribute.deleteMany({where:{categoryId:id}});if(attributeIds.length)await tx.categoryAttribute.createMany({data:attributeIds.map(attributeId=>({categoryId:id,attributeId}))});}return tx.category.findUnique({where:{id},include})});if(existing?.image&&existing.image!==category.image){const publicId=publicIdFromCloudinaryUrl(existing.image);if(publicId)await deleteImage(publicId).catch(()=>{});}invalidatePublicCache('categories','products','homepage','seo');return Response.json(category)}catch(error){return jsonError(error)}}
export async function DELETE(request){try{await requirePermission('categories.delete');const id=new URL(request.url).searchParams.get('id');const count=await prisma.product.count({where:{categoryId:id,active:true}});if(count)throw new Error('Archive or move products before deleting this category.');const category=await prisma.category.update({where:{id},data:{active:false}});invalidatePublicCache('categories','products','homepage','seo');return Response.json(category)}catch(error){return jsonError(error)}}
