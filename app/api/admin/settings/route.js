...
export async function PUT(request){try{await requirePermission('settings.edit');const data=schema.parse(await request.json());return Response.json(await prisma.siteSettings.upsert({where:{id:'default-settings'},create:{id:'default-settings',...data},update:data}))}catch(error){return jsonError(error)}}
