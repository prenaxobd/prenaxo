...
export default function AdminLoginForm({ inviteToken = '', initialEmail = '' }) {
  ...
  async function submit(event) {
    ...
    setError('');
    try {
      const response = await fetch('/api/admin/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password, rememberMe, inviteToken }) });
  ...
  return <main className="admin-login-page"><form className="admin-login-card" onSubmit={submit}>
    ...
    <label>Email Address<input type="email" value={email} onChange={event => setEmail(event.target.value)} autoComplete="email" readOnly={Boolean(inviteToken)} required /></label>
