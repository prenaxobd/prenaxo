...
export async function createSession(userId) {
	...
	(await cookies()).set('prenaxo_session', token, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', maxAge: 604800, path: '/' });
}
export async function getCurrentUser() {
	try {
		if (secret) {
			const cookieStore = await cookies();
			const token = cookieStore.get('prenaxo_session')?.value || cookieStore.get('khatibazar_session')?.value;
			if (token) {
...
const ADMIN_SESSION_COOKIE = 'prenaxo_admin_session_v2';
export const MAIN_ADMIN_EMAIL = 'prenaxo@gmail.com';

export function isPermanentAdminEmail(email) {
...
export async function createAdminSession(userId) {
	...
	await prisma.adminAuthSession.create({ data: { userId, tokenHash: hashToken(token), expiresAt: new Date(Date.now() + maxAge * 1000), rememberMe: false, ...context } });
	(await cookies()).set(ADMIN_SESSION_COOKIE, token, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: '/' });
}
...
export async function getAdminSession() {
	try {
		...
		const hasActiveRole = session?.user?.adminRoles?.some(assignment => assignment.role?.isActive);
		const isPermanentAdmin = isPermanentAdminEmail(session?.user?.email);
		if (!session || session.revokedAt || session.expiresAt <= new Date() || session.user.role !== 'ADMIN' || (!session.user.adminAuthRole && !isPermanentAdmin) || !session.user.adminActive || (!hasActiveRole && !isPermanentAdmin)) return null;
