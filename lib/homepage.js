...
export async function getHomepageData() {
  ...
  const [banners, categories, products, reviews, brands] = await Promise.all([
    ...
    prisma.brand.findMany({ where: { active: true, products: { some: { active: true } } }, orderBy: [{ featured: 'desc' }, { name: 'asc' }] }).catch(() => []),
  ]);
  ...
  return { banners, categories, featured, topSelling: topSelling.length ? topSelling : serialized.slice(0, 8), deals, newArrivals: serialized.slice(0, 8), categorySections, reviews, sections, brands, products: serialized };
}
