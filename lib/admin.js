import { getAdminSession, getCurrentUser } from './auth';
...
export async function getUserPermissions(userId) {
  ...
  if (user.adminAuthRole === 'main_admin') return new Set(ALL_ADMIN_PERMISSIONS);

  const activeAssignments = (user.adminRoles || []).filter(
    assignment => assignment.isActive && assignment.role?.isActive
  );

  // Preserve access for legacy ADMIN users that predate role assignment,
  // while respecting explicit RBAC assignments when they exist.
  if (!activeAssignments.length) return new Set(ALL_ADMIN_PERMISSIONS);
